...
function ReplaceLiteralOnce($Text, $Old, $New, $Label) {
    $already = CountLiteral $Text $New
    if ($already -gt 0) {
        Warn "$Label already patched"
        return $Text
    }

    $count = CountLiteral $Text $Old
    if ($count -eq 0) {
        throw "$($Label)_PATTERN_NOT_FOUND"
    }
    if ($count -gt 1) {
        throw "$($Label)_PATTERN_NOT_UNIQUE_COUNT_$count"
    }
...
$BackupFile = "$TargetFile.bak_TRANSLATOR_PATCH_V3_2_$Timestamp"
$TranslatorBackupFile = "$TranslatorFile.bak_TRANSLATOR_PATCH_V3_2_$Timestamp"
...
'@
    ...
    # -----------------------------
    # Hook 2 (patch redact line)
    # -----------------------------
    $hook2Old = '                        msg = _pp200_redact(str(msg))'
    $hook2New = '                        msg = _pp200_redact(str(translate_api_message(msg)))'
    ...
    if (-not $content.Contains('msg = _pp200_redact(str(translate_api_message(msg)))')) {
    ...
    Ok "PATCH V3.2 COMPLETED SUCCESSFULLY"
