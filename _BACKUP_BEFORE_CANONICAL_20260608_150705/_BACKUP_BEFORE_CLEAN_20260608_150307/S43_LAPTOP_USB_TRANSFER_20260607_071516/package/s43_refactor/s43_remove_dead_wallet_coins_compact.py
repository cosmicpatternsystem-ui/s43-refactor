...
for node in ast.walk(run_node):
    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == REMOVE:
        defs.append(node)

    if isinstance(node, ast.Call):
        f = node.func
        if isinstance(f, ast.Name) and f.id == REMOVE:
            calls.append(node)
        elif isinstance(f, ast.Attribute) and f.attr == REMOVE:
            calls.append(node)

    if isinstance(node, ast.Name) and node.id == REMOVE:
        name_refs.append(node)

    if isinstance(node, ast.Attribute) and node.attr == REMOVE:
...
backup = TARGET.with_name(f"s43.py.before_remove_wallet_coins_compact_{stamp}")
shutil.copy2(TARGET, backup)
print(f"Backup: {backup}")
print(f"Removing {REMOVE}: lines {start}-{end}")
...
print("OK: _wallet_coins_compact removed and s43.py compiles.")
