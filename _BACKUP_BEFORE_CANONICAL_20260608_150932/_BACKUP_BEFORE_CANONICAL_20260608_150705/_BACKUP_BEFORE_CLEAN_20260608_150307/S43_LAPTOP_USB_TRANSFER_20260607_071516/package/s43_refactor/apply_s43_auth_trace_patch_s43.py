...
patched = src
patched = must_replace(patched, old_detect, new_detect, "_arzplus_detect_403_cause")
patched = must_replace(patched, old_log, new_log, "_arzplus_log_403")
patched = must_replace(patched, old_auth, new_auth, "AUTHDBG_HEADER")

ts = datetime.now().strftime("%Y%m%d_%H%M%S")
backup = target.with_name(f"s43_before_auth_trace_{ts}.py")
