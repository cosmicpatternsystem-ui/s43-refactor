...
$outDir = Join-Path $root ("safe_patch_proposal_full_fixed_" + $ts)
...
$proposal.Add("2. Open PATCH_HITS.csv and filter by these keywords:")
...
$proposal.Add("- Reason: instrumented candidate and better suited for evidence-first debugging")
$proposal.Add("")
...
$notes.Add("")
$notes.Add("Important:")
$notes.Add("- 11029.py should remain reference-only unless explicitly decided otherwise")
...
$notepad.Add("   Hashes for audit and reproducibility")
$notepad.Add("")
...
$zipPath = Join-Path $root ("safe_patch_proposal_full_fixed_" + $ts + ".zip")
